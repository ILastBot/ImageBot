...
${videos.map(video2 => `[**${++index}**] **${video2.title}**`).join('\n')}`)//@〖D̷o̷̶Ḿa̶ |̷ B̷O̷Y̷〗#1362  حقوق
 ...
 //@〖D̷o̷̶Ḿa̶ |̷ B̷O̷Y̷〗#1362  حقوق
        ...
        if (!serverQueue) return msg.channel.send('**لا يوجد شيء حالي في العمل.**');//@〖D̷o̷̶Ḿa̶ |̷ B̷O̷Y̷〗#1362  حقوق
...
function play(guild, song) {//@〖D̷o̷̶Ḿa̶ |̷ B̷O̷Y̷〗#1362  حقوق
  ...
  .addField("Download ", `[**Download MP3**](https://www.flvto.biz/sa/downloads/mp3/yt_${video.id})`, true)//@〖D̷o̷̶Ḿa̶ |̷ B̷O̷Y̷〗#1362  حقوق
    ...
    .setThumbnail('http://cdn.akhbaar24.com/430e061a-f89a-43c7-86d9-82fae5f7c495.jpg')//@〖D̷o̷̶Ḿa̶ |̷ B̷O̷Y̷〗#1362  حقوق
      .setColor('RANDOM')//@〖D̷o̷̶Ḿa̶ |̷ B̷O̷Y̷〗#1362  حقوق
...
client.on('message', msg => {//msg
    if (msg.content === '+colors') {
      msg.channel.send({file : "https://4.top4top.net/p_1249q63e71.png"})
    }
  });
...
❖ +auto role set [NameRole] ➾لتفعيل اعطاء الرتبة التلقائية 
❖ +info role      ➾ لمعرفة الرتبة المفعلة                    
❖ +role @user <rank> ➾ لأعطاء رتبة لعضو معين
❖ +roleRemove @user <rank> ➾ لحدف رتبة من عضو معين
...
if(message.content.startsWith(`+auto`)) {
  ...
  let perms = message.member.hasPermission(`MANAGE_ROLES`)
    if(!perms) return message.reply(`**ليس لديك الصلاحية**`)
  if(!perms) return message.reply(`You don't have permissions, required permission : Manage Roles.`)
 let args = message.content.split(" ").slice(1)
 if(!args.join(" ")) return message.reply(`${prefix}auto role set [ROLE NAME]`)
 let state = args[0]
 if(!state.trim().toLowerCase() == 'role' || !state.trim().toLowerCase() == 'setrole') return message.reply(`Please type a right state, ${prefix}modlogs role set [ROLE NAME]`)
   if(state.trim().toLowerCase() == 'role') {
